...
class DashboardRepository {
  ...
  DashboardRepository({
    ...
    this.staleDuration = const Duration(minutes: 2), // 2 minutes for Dashboard
  ...
  Future<Map<String, dynamic>> getDashboardData({
    ...
    if (!forceRefresh &&
      ...
      // Start background refresh if getting close to expiry (last 30 seconds)
    ...
    if (!forceRefresh) {
      try {
        final cache = await _persistentCacheService.getDashboardCache(key);
        ...
        logError(
            error: e,
            stackTrace: st,
            source: debugSource ?? 'DashboardRepository.persistentCache');
      }
    }
  ...
  Future<Map<String, dynamic>> _fetchAndCache(
    ...
    try {
      ...
      await _persistentCacheService.setDashboardCache(
        key,
        DashboardCacheEntry(data: freshData, fetchedAt: now),
      );
      ...
      logError(
          error: e,
          stackTrace: st,
          source: debugSource ?? 'DashboardRepository.fetch');
      rethrow;
    }
  }
  ...
  void _refreshInBackground(
    ...
    _fetchAndCache(key, fetcher, 'DashboardRepository.backgroundRefresh')
        .catchError((error) {
